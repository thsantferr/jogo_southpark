<?php 
	...
	$user = $_POST['username']; 
	$pass = $ $_POST['pass']; 
	$sql = 'SELECT user pass FROM users WHERE user = ? and pass = ?';
	try{
		...
		echo $e->getMessage();}
	$veruser = $resultado->user;
	$verpass = $resultado->pass;
	if($veruser == $user and $pass == $verpass){
...
function Confirma(idquest){
	resposta = confirm("deseja remover essa pessoa?");
	if(resposta == true){
		return true;
		}
		return false;	
